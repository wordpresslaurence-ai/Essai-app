<div x-data="{ confirmerSuppression: false }">
    <a href="{{ route('pipeline.index') }}" class="lb-muted" style="font-size:13px;text-decoration:none" wire:navigate>← Retour au pipeline</a>

    <h1 class="lb-h1 mt-4">{{ $opportunite ? "Modifier l'opportunité" : 'Nouvelle opportunité' }}</h1>
    ...
    <form wire:submit="enregistrer" class="lb-card mt-5" style="padding:24px;max-width:640px">
        <div>
            ...
            <input id="titre" type="text" wire:model="titre" class="lb-field" placeholder="ex. Accompagnement coaching">
            ...
            <div>
                ...
                <select id="contact_id" wire:model="contact_id" class="lb-field">
                    ...
                    @foreach ($contacts as $c)
                        <option value="{{ $c->id }}">{{ $c->nomComplet() }}</option>
                    @endforeach
                </select>
                @error('contact_id') <p class="lb-error">{{ $message }}</p> @enderror
            </div>
            <div>
                <label for="montant" class="lb-label">Montant (€)</label>
                <input id="montant" type="number" step="0.01" min="0" wire:model="montant" class="lb-field" placeholder="0,00">
                @error('montant') <p class="lb-error">{{ $message }}</p> @enderror
            </div>
        </div>

        <div class="mt-5 max-w-xs">
            <label for="etape" class="lb-label">Étape</label>
            <select id="etape" wire:model="etape" class="lb-field">
                @foreach ($libelles as $cle => $lib)
                    <option value="{{ $cle }}">{{ $lib }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="flex items-center justify-between mt-7">
            ...
            <div class="flex items-center gap-3">
                <a href="{{ route('pipeline.index') }}" class="lb-muted" style="font-size:13.5px;text-decoration:none" wire:navigate>Annuler</a>
    ...
    @if ($opportunite)
        <div x-cloak x-show="confirmerSuppression" x-transition class="lb-modal-backdrop" x-on:keydown.escape.window="confirmerSuppression = false">
            <div class="lb-modal" x-on:click.outside="confirmerSuppression = false">
                <h3 class="lb-h3" style="font-size:20px">Supprimer cette opportunité ?</h3>
